...
class TurboQuantWrapper:
    ...
    def _last_cuda_error(self) -> str:
        raw = self._lib.tq_last_error()
        return raw.decode() if raw else ""

    def _kernel_raise(self, rc: int, scheme: str, op: str) -> None:
        """Raise RuntimeError with a readable message for any failed kernel call.

        rc == -2 means a CUDA error; tq_last_error() has the details.
        rc == -1 means an invalid argument was passed to the C API.
        """
        if rc == -2:
            raise RuntimeError(f"{scheme} {op} CUDA error: {self._last_cuda_error()}")
        raise RuntimeError(f"{scheme} {op} failed (rc={rc})")
    ...
    def pack(
        ...
        if rc != 0:
            self._kernel_raise(rc, "turbo_prod", "pack_kv")
    ...
    def dequant(
        ...
        if rc != 0:
            self._kernel_raise(rc, "turbo_prod", "dequant_kv")
    ...
    def fused_attn_logits(
        ...
        if rc != 0:
            self._kernel_raise(rc, "turbo_prod", "fused_attn_logits")
    ...
    def fused_attn_output(
        ...
        if rc != 0:
            self._kernel_raise(rc, "turbo_prod", "fused_attn_output")
    ...
    def mse_pack(
        ...
        if rc != 0:
            self._kernel_raise(rc, "turbo_mse", "pack_kv")
    ...
    def mse_dequant(
        ...
        if rc != 0:
            self._kernel_raise(rc, "turbo_mse", "dequant_kv")
